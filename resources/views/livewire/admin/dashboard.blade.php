<div>
    ...
    <div class="mt-8 grid gap-6 lg:grid-cols-2">
        <section class="rounded-[2rem] bg-white p-6 shadow-sm">
            <h2 class="text-xl font-semibold">Последние обращения</h2>
            <div class="mt-4 divide-y divide-slate-100">
                @forelse ($latestAppeals as $appeal)
                    <div class="py-3">
                        <p class="font-medium">{{ $appeal->registered_number }} · {{ $appeal->topic }}</p>
                        <p class="text-sm text-slate-500">{{ $appeal->name }} · {{ $appeal->status }}</p>
                    </div>
                @empty
                    <p class="text-slate-500">Нет обращений.</p>
                @endforelse
            </div>
        </section>
            ...
            <div class="mt-4 divide-y divide-slate-100">
                @forelse ($latestNews as $post)
                    <div class="py-3">
                        ...
                        <p class="text-sm text-slate-500">{{ $post->status }} · {{ $post->category }}</p>
                    </div>
                @empty
                    <p class="text-slate-500">Нет новостей.</p>
                @endforelse
            </div>
        </section>
    </div>
