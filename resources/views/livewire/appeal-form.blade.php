<form wire:submit="submit" class="rounded-[2rem] border border-slate-200 bg-white/95 p-6 shadow-sm shadow-slate-200/60 backdrop-blur md:p-8">
    <div class="mb-6">
        <p class="text-sm font-medium uppercase tracking-[0.3em] text-blue-800">Общественная приёмная</p>
    ...
    <div class="grid gap-4 md:grid-cols-2">
        ...
        <label class="block text-sm font-medium text-slate-700">Адрес / место проблемы <span class="text-slate-400">необязательно</span>
        ...
        <label class="block text-sm font-medium text-slate-700 md:col-span-2">Тема обращения
            <input wire:model="topic" class="mt-2 w-full rounded-2xl border-slate-200 bg-slate-50 px-4 py-3 outline-none transition focus:border-blue-700 focus:bg-white focus:ring-4 focus:ring-blue-100" placeholder="Благоустройство, ЖКХ, транспорт" />
    ...
    <label class="mt-4 block text-sm font-medium text-slate-700">Прикрепить файлы
        ...
        @error('attachments.*')<span class="mt-1 block text-xs text-red-600">{{ $message }}</span>@enderror
    </label>

    <label class="mt-5 flex gap-3 rounded-2xl border border-slate-200 bg-slate-50 p-4 text-sm text-slate-700">
        <input wire:model="consent" type="checkbox" class="mt-1 rounded border-slate-300 text-blue-950 focus:ring-blue-200" />
        <span>Я согласен на обработку персональных данных и подтверждаю достоверность указанных контактов.</span>
    </label>
    @error('consent')<span class="mt-1 block text-xs text-red-600">{{ $message }}</span>@enderror

    <button type="submit" class="mt-6 inline-flex w-full items-center justify-center rounded-2xl bg-blue-950 px-6 py-4 text-sm font-semibold text-white transition hover:-translate-y-0.5 hover:bg-blue-900 focus:outline-none focus:ring-4 focus:ring-blue-100 md:w-auto">
