<x-public.layout :title="$post->seo_title ?: $post->title" :description="$post->seo_description ?: $post->excerpt">
    <article class="mx-auto max-w-4xl px-4 py-16 sm:px-6 lg:px-8">
        ...
        <section class="mt-12">
            <h2 class="text-2xl font-semibold">Фотогалерея</h2>
            <div class="mt-5 grid gap-4 md:grid-cols-3">
                @for ($i = 1; $i <= 3; $i++)
                    <div class="h-44 rounded-[2rem] bg-slate-200"></div>
                @endfor
            </div>
        </section>
        ...
        @if (($related ?? collect())->isNotEmpty())
            <section class="mt-12">
                <h2 class="text-2xl font-semibold">Похожие материалы</h2>
                <div class="mt-5 grid gap-4 md:grid-cols-3">
                    @foreach ($related as $item)
                        <a href="{{ route(request()->routeIs('news.show') ? 'news.show' : 'blog.show', $item) }}" class="rounded-3xl border border-slate-200 bg-white p-5 transition hover:-translate-y-1">
                            <p class="text-sm text-red-600">{{ $item->category }}</p>
